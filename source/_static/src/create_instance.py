...
import json
import logging
import random  # Added by zhangyazhe at 2013/11/27
import string  # Added by zhangyazhe at 2013/11/27
...
from openstack_dashboard.dashboards.project_resource.images_and_snapshots import utils

from openstack_dashboard.dashboards import log_common

LOG = logging.getLogger(__name__)
ceclog = logging.getLogger("cecgw." + __name__)
...
class SelectProjectUser(workflows.Step):
    ...
    # 贡献了两个字段(从view处获取初始值), 供其他steps depends_on
    contributes = ("project_id", "user_id")


class SetInstanceDetailsAction(workflows.Action):
    # Action 继承自django forms.Form类，因此，本质上只是一个更加复杂的表单。
    SOURCE_TYPE_CHOICES = (
        ('', _("--- Select source ---")),
    ...
    class Meta:
        name = _("Details")
        help_text_template = ("project_resource/instances/"
    ...
    def clean(self):
        # 重写django forms.Form类的clean 方法，对表单输入数据进行校验。
    ...
    # Action父类有一个_populate_choices方法，会调用所有的"populate_%s_choices"函数，
    # "populate_%s_choices" 函数的作用是动态获取下拉列表可供选择的数据。
...
class SetInstanceDetails(workflows.Step):
    ...
    linkafter = ("launch_instance__setaccesscontrolsaction")######
    linkbefore = "stop"
    depends_on = ("project_id", "user_id")###add by zhaoyuanjie 
...
#KEYPAIR_IMPORT_URL = "horizon:project_system_management:keypair_management:keypairs:import"
KEYPAIR_IMPORT_URL = "horizon:project_user:keypair_management:keypairs:import"
...
class SetAccessControlsAction(workflows.Action):
    ...
    groups = forms.MultipleChoiceField(label=_("Security Groups"),
                                       ...
                                       help_text=_("Launch instance in these "
                                                   "security groups."))
    # Added by zhangyazhe at 2013/11/25
    encryption = forms.ChoiceField(label=_("Instance Encryption"), widget=forms.Select,
...
class SetAccessControls(workflows.Step):
    ...
    linkafter = ("launch_instance__setnetworkaction")######
    linkbefore = ("launch_instance__setinstancedetailsaction")########## add by zyj
    def contribute(self, data, context):
        if data:
            ...
            context['encryption'] = data.get("encryption", "") # Added by zhangyazhe at 2013/11/27
        return context
...
class PostCreationStep(workflows.Step):
    ...
    linkbefore = ("launch_instance__setnetworkaction")##########
    linkafter = "stop"########
...
class SetNetwork(workflows.Step):
    ...
    linkafter = ("launch_instance__customizeaction")######
    linkbefore = ("launch_instance__setaccesscontrolsaction")##########
    ...
    if api.neutron.is_port_profiles_supported():
        ...
        template_name = "project_resource/instances/_update_networks.html"
...
class LaunchInstance(workflows.Workflow):
    ...
    success_url = "horizon:project_resource:instances:index"
    # 接下来，在workflow中，一共有五个steps，其中，第一个步骤SelectProjectUser
    # 设置permissions而进行隐藏。它的主要作用是贡献两个字段让后面的步骤进行依
    # 赖，从而对后面的steps进行校验。
    ...
    def handle(self, request, context):
        ...
        # Added by zhangyazhe at 2013/11/27
        if context["encryption"] == "yes":
            is_encrypted = 1
        ...
        try:
            api.nova.server_create(request,
                                   ...
                                   secret=secret, # Added by zhangyazhe at 2013/11/21
                                   is_encrypted=is_encrypted) # Added by zhangyazhe at 2013/11/21
                                   # admin_pass=context['admin_pass'])
            m = log_common.get_info(request, u"创建虚拟机%s" % context['name'], log_common.STATUS_SUCCESS)
            ceclog.info(m)
            return True
        except Exception:
            m = log_common.get_info(request, u"创建虚拟机%s" % context['name'], log_common.STATUS_FAIL)
            ceclog.error(m)
            exceptions.handle(request)
            return False
